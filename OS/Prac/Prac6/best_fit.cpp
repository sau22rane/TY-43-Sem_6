...
void best_fit(vector<Process> *processes, vector<Mem_Block*> *rem_mem_blocks, vector<Mem_Block*> *all_mem_blocks);
void get_input(vector<Process> *processes, vector<Mem_Block*> *rem_mem_blocks, vector<Mem_Block*> *all_mem_blocks);
bool Sort_Rem_Mem(const Mem_Block* a, const Mem_Block* b);
...
int main(){
    ...
    best_fit(&processes, &rem_mem_blocks, &all_mem_blocks);
...
void best_fit(vector<Process> *processes, vector<Mem_Block*> *rem_mem_blocks, vector<Mem_Block*> *all_mem_blocks){
    ...
    for (auto pr = (*processes).begin(); pr != (*processes).end(); ++pr){
        ...
        sort((*rem_mem_blocks).begin(), (*rem_mem_blocks).end(), Sort_Rem_Mem);
    
        for (auto mem = (*rem_mem_blocks).begin(); mem != (*rem_mem_blocks).end(); ++mem){
            if((*mem)->size>=pr->size){
