...
int main(){
    ...
    pthread_create(&ps_sch,NULL, fcfs::processScheduler,(void *)0);
...
void *fcfs::processScheduler(void *i){
    time_t elapsed;
    int count = 0;
    Gantt *temp;
    while (1) {
...
void fcfs::displaySchedule(){
    cout<<endl<<"First Come First Serve Schedule:\n";
    cout<<"------------------------------------------------\n";
    cout<<"|Process|  AT\t|  BT\t| Fin T\t|  TAT\t|  WT\t|\n";
    cout<<"------------------------------------------------\n";
    float avg_TAT=0, avg_WT=0;
    for(int i = 0;i<5;i++){
        cout << "|  "<<ps[i].ProcessID<<"\t|   "<<ps[i].AT << "\t|   " << ps[i].BT<< "\t|   " << ps[i].FT<< "\t|   " << ps[i].TAT<<"\t|   "<<ps[i].WT << "\t|\n";
