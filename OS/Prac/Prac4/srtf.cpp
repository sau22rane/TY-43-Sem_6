...
int main(){
    ...
    pthread_create(&ps_sch,NULL, srtf::processScheduler,(void *)0);
    pthread_join(ps_gen,NULL);
    pthread_join(ps_sch,NULL);
    srtf::displaySchedule();
    srtf::displayGantt();
    return 0;
...
bool Process::operator()(Process * const &obj1, Process * const &obj2){
    if(obj1->RBT==obj2->RBT)
        return (obj1->AT >= obj2->AT);
    return (obj1->RBT > obj2->RBT);
}
...
void *srtf::processScheduler(void *i){
    time_t elapsed;
    Process *p, *prev=NULL;
    int count = 0;
    Gantt *temp;
    while (1) {
        ...
        if(!Q.empty() || prev!=NULL){
            if(!Q.empty()){
                ...
                if(prev!=NULL && prev->RBT>0 && (prev->RBT <= p->RBT)){
                    Q.push(p);
                    p = prev;
                }
...
void srtf::displaySchedule(){
    cout<<endl<<"Shortest Remaining Time First Schedule:\n";
    cout<<"----------------------------------------------------------\n";
