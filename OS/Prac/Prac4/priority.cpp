...
int main(){
    ...
    pthread_create(&ps_sch,NULL, priority::processScheduler,(void *)0);
    pthread_join(ps_gen,NULL);
    pthread_join(ps_sch,NULL);

    priority::displaySchedule();
    priority::displayGantt();
...
void *priority::processScheduler(void *i){
    time_t elapsed;
    int count = 0;
    Gantt *temp;
    while (1) {
...
void priority::displaySchedule(){
    cout<<endl<<"Priority Schedule:\n";
    cout<<"----------------------------------------------------------\n";
